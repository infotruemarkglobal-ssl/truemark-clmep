...
--   postgres has rolbypassrls = true, so it skips ALL RLS policies
...
-- postgres retains full access for migrations and admin operations.
...
-- ── Grant neondb_app to postgres ─────────────────────────────────────────
-- Required so postgres can switch roles via set_config('role', ...).
